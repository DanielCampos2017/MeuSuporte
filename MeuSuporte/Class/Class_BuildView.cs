...
{
    class Class_BuildView
    {
        private MainForm _MainForm;
        string GitHubRepo = "DanielCampos2017/MeuSuporte";

        public Class_BuildView(MainForm Form_)
        {
            _MainForm = Form_;
        }
        ...
        private void CheckBuild()
        {
            ...
            {
                _MainForm.Text = $"MeuSuporte Build {BuildLocal} - checking";
            }));
            ...
            else
            {
                ...
                _MainForm.Invoke(new Action(() =>
                {
                    _MainForm.Text = $"MeuSuporte Build {BuildLocal} - No Connection";
                    _MainForm.PainelInfoDescricao(Resources.Offline_Black, $"Sem Conexão:\r\nNão foi possível conectar ao repositório GitHub. https://github.com/{GitHubRepo}/releases/latest");
                }));
            ...
            if (vLatest == new Version(0, 0, 0, 0))
            {
                ...
                _MainForm.Invoke(new Action(() =>
                {
                    _MainForm.Text = $"MeuSuporte Build {BuildLocal} - Unknown";  // Versão não encontrada no GitHub
                    _MainForm.PainelInfoDescricao(Resources.Alert_Black, $"404 Not Found!:\r\nNão foi possível encontrar repositório. https://github.com/{GitHubRepo}/releases/latest");
                }));
            }
            else
            {
                if (result > 0)
                {
                    ...
                    _MainForm.Invoke(new Action(() =>
                    {
                        _MainForm.Text = $"MeuSuporte Build {BuildLocal} - Beta"; // Versão local maior que a do GitHub
                        _MainForm.PainelInfoDescricao(Resources.Code_Black, $"Verão Beta\r\nEssa é uma versão de desenvolvimento. Que ainda se encontra em desenvolvimento e não trata de uma versão final !");
                    }));
                }
                else if (result < 0)
                {
                    ...
                    _MainForm.Invoke(new Action(() =>
                    {
                        _MainForm.checkBoxAllState(false);
                        _MainForm.Text = $"MeuSuporte Build {BuildLocal} - Legacy"; // Versão local inferior à do GitHub                       
                        _MainForm.PainelInfoDescricao(Resources.Dawnload_Black, $"Atualização Disponível!:\r\nPara garantir o melhor eficiência baixe últimas versão. https://github.com/{GitHubRepo}/releases/latest");
                    }));
                }
                else
                {
                    ...
                    _MainForm.Invoke(new Action(() =>
                    {                        
                        _MainForm.Text = $"MeuSuporte Build {BuildLocal} - Stable"; // Versão local igual à do GitHub
                        _MainForm.PainelInfoDescricao(Resources.Security_Black, $"Estável:\r\nSua verão se encontrar na mesma versão do repositório. https://github.com/{GitHubRepo}/releases/latest");
                    }));
                }
            }
        }
